module.exports = {
  ...
  run: async (bot, message, args) => {
    ...
    if (!role)
      return message.channel.send(
        "Veuillez fournir un rôle à ajouter à l'utilisateur."
      );
    ...
    if (!message.guild.me.hasPermission(["MANAGE_ROLES", "ADMINISTRATOR"]))
      return message.channel.send(
        "Je n'ai pas l'autorisation d'utiliser cette commande."
      );
    ...
    if (rMember.roles.cache.has(role.id)) {
      ...
      message.channel
        .send(`Le rôle, ${role.name}, a été procuré à ${rMember.displayName}.`)
        .then(msg => {
